...
import numpy as np
import transforms
from model import Mobile_HR_ViT
from model import model20230811 as HRMS
...
def create_model(num_joints, load_pretrain_weights=False):
    # model = Mobile_HR_ViT(base_channel=32, num_joints=num_joints)
    model = HRMS.HR_MiT_Snet(base_channel=16, num_joints=num_joints)
    # model =hrnet.HRNet(c=48, nof_joints=17, bn_momentum=0.1)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model.to(device)
    # model = LiteHRNet
    print("整体参数量", count_parameters(model))

    if load_pretrain_weights:
        # 载入预训练模型权重
        # 链接:https://pan.baidu.com/s/1Lu6mMAWfm_8GGykttFMpVw 提取码:f43o
        weights_dict = torch.load("./imagenet/hrnet_w32-36af842e.pth", map_location='cpu')
...
def main(args):
    ...
    heatmap_hw = (args.fixed_size[0] // 8, args.fixed_size[1] // 8)
...
if __name__ == "__main__":
    ...
    # 训练数据集的根目录(coco2017)
    parser.add_argument('--data-path', default=r'D:\1-DATA OF AI\coco2017', help='dataset')
    # COCO数据集人体关键点信息
